...
public struct LLMParameters: Sendable {
    ...
    let nLength: Int
    /// Indicates wether the BOS token is added by the ``LLM``.
    let addBos: Bool
    ...
    ///
    /// - Parameters:
    ///   - nLength: The maximum output length generated by the ``LLM``, defaults to `128`.
    ///   - addBos: Indicates wether the BOS token is added by the ``LLM``, defaults to `false`.
    ///   - nGpuLayers: Number of layers to store in VRAM, defaults to `1`, meaning Apple's `Metal` framework is enabled.
